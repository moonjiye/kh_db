...
-- 별칭 설정 하기 : AS, 열이름 또는 컬럼을 별칭으로 표시할 수 있음
SELECT ENAME AS 이름,SAL AS 급여, SAL*12+COMM AS 연봉,COMM AS 성과급
FROM EMP;

-- 중복 제거 : DISTINCT, 데이터를 조회할 때 값이 중복되는 행이 어려개 조회되는 경우, 값이 중복된 행을 한개씩만 선택하고하는 경우 사용
...
-- 급여가 3000 이상이고 부서가 20번이거나 입사일이 82년 1월 1일 이전 사원 조회
SELECT * 
FROM EMP
...
-- IN 연산자 : 특정 열 데이터 값만을 조회하고자 할 때 사용
SELECT * 
FROM EMP
WHERE JOB = 'MANAGER'
    OR JOB = 'SALESMAN'
...
-- NULL : 값이 정해지않음, 알 수 없는 값을 의미함, 그래서 연산, 할당, 비교 불가(=,IN)
